...
export const getBookingById = async (req, res, next) => {
  try {
    ...
    // Scope: this route runs behind authenticateUser only, so req.user is the
    // customer JWT payload (it has no `.role` field — the old check compared
    // against a property that never existed, so it always evaluated to false
    // and silently let ANY logged-in user view ANY booking by id). Vendors and
    // admins have their own scoped endpoints (getVendorBookings /
    // getAllBookingsAdmin), so here we only need to guard the customer case.
...
export const updateBookingStatus = async (req, res, next) => {
  try {
    ...
    // This endpoint is mounted for both vendors (/vendor/bookings/:id/status)
    // and admins (/admin/bookings/:id/status) with the same controller, but
    // had no ownership check at all — any active vendor could change the
    // status of ANY booking, not just their own property's bookings.
    if (req.vendor && booking.vendor_id !== req.vendor.id) {
    ...
    const actor = req.admin ? "admin" : req.vendor ? "vendor" : "user";
    ...
    const releasesInventory =
      ["cancelled", "no_show"].includes(finalBookingStatus) &&
    ...
    try {
      ...
      // Previously only the dedicated cancel endpoints released inventory —
      // moving a booking to cancelled/no_show through this status endpoint
      // left the rooms/dorm beds permanently blocked.
...
export const cancelBooking = async (req, res, next) => {
  ...
  // req.user.role never existed on any auth payload in this app — user auth
  // sets req.user (JWT, no role field), vendor auth sets req.vendor (session),
  // admin auth sets req.admin (session). Derive the actor from whichever
  // middleware actually ran instead of reading a field that was always
  // undefined (which made the ownership check below a no-op for customers,
  // and made vendor cancellation throw because req.user was undefined).
  const cancelledBy = req.admin ? 'admin' : req.vendor ? 'vendor' : 'user';
  ...
  try {
    ...
    if (cancelledBy === 'vendor' && booking.vendor_id !== req.vendor.id) {
      throw httpError(403, 'Forbidden');
    }
