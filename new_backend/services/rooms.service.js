...
export const deleteOldRooms = async (connection, propertyId) => {
    // ON DELETE CASCADE on rooms -> room_beds, room_dorm_beds, room_prices,
    // room_availability, and (after the migration) room_property_images
    // all clean themselves up when the room row goes.
    await connection.execute(
        `DELETE FROM rooms WHERE property_id=?`,
        [propertyId]
    );
...
/**
 * CHANGED: no longer takes multer `files`. Takes the ids of photos the
 * vendor picked from the property's existing photo pool (already validated
 * as belonging to this property up in the controller) and links them via
 * room_property_images.
 *
 * imageIds: number[] — property_images.id values, in display order
 * coverImageId: number|null — which of those ids is the room's cover photo;
 *               falls back to the first id if omitted or not in the list
 */
export const saveRoomImages = async (connection, roomId, imageIds = [], coverImageId = null) => {
    if (!imageIds.length) return;

    const resolvedCoverId = imageIds.includes(coverImageId) ? coverImageId : imageIds[0];

    for (let i = 0; i < imageIds.length; i++) {
        await connection.execute(
            `
            INSERT INTO room_property_images (room_id, property_image_id, is_cover, sort_order)
            VALUES (?,?,?,?)
            `,
            [
                roomId,
                imageIds[i],
                imageIds[i] === resolvedCoverId ? 1 : 0,
                i
            ]
        );
    }
...
export const validateRooms = (rooms) => {
    ...
    for (const room of rooms) {
        ...
        // NEW: every room needs 1-5 photos picked from the property's pool
        const imageIds = room.image_ids;
        if (!Array.isArray(imageIds) || imageIds.length === 0) {
            throw new Error(`${room.room_name} needs at least one photo selected.`);
        }
        if (imageIds.length > 5) {
            throw new Error(`${room.room_name} can have at most 5 photos.`);
        }
    }
};
...
export const saveCompleteRoom = async (connection, propertyId, room) => {
    const roomId = await createRoom(connection, propertyId, room);
    await saveRoomPrice(connection, roomId, room);
    await saveRoomBeds(connection, roomId, room.beds);

    if (room.room_category === "dorm") {
        await saveDormBeds(connection, roomId, room.dorm_beds);
    }

    // CHANGED: was files + cover_index, now image_ids + cover_image_id
    await saveRoomImages(
        connection,
        roomId,
        room.image_ids,
        room.cover_image_id ?? null
    );
