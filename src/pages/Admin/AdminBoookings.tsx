      ...
      <div className="flex items-center gap-1.5 mb-4 flex-wrap">
        {STATUS_TABS.map((t) => (
          <button
            ...
            className={`px-3.5 py-1.5 rounded-full text-[12.5px] font-medium transition border ${status === t.key
                        ...
                        <td className="px-3 py-3 relative">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setPaymentMenuFor(
                                paymentMenuFor === b.id ? null : b.id
                              );
                            }}
                            className="flex items-center gap-1 text-[10.5px] font-semibold uppercase tracking-[0.04em] rounded-full px-2 py-0.5"
                            style={{
                              background: paymentMeta.bg,
                              color: paymentMeta.text,
                            }}
                          >
                            {paymentMeta.label}
                            <ChevronDown size={10} />
                          ...
                          {paymentMenuFor === b.id && (
                            ...
                            >
                              {Object.keys(PAYMENT_META).map((status) => (
                                <button
                                  key={status}
                                  onClick={() => {
                                  ...
                                  className="w-full text-left px-3 py-2 text-[12px] hover:bg-[#F5F2EA]"
                                >
                                  {PAYMENT_META[status as PaymentStatus].label}
                                </button>
                              ))}
                            </div>
                          )}
