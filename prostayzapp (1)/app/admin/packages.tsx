...
type Pkg = { id: string; title: string; destination: string; duration_nights: number; duration_days: number; price: number; rating: number; category: string; cover_image: string };
...
export default function AdminPackagesList() {
  ...
  const load = useCallback(async () => {
    try { setPackages(await api<Pkg[]>("/admin/packages", { auth: true })); }
  ...
  const handleDelete = (id: string, title: string) => {
    Alert.alert("Delete package?", `Remove "${title}" permanently?`, [
      { text: "Cancel", style: "cancel" },
      {
        text: "Delete", style: "destructive", onPress: async () => {
          try { await api(`/admin/packages/${id}`, { method: "DELETE", auth: true }); await load(); }
          catch (e: any) { Alert.alert("Error", e.message); }
        }
      },
    ]);
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + 10 }]}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Holiday Packages</Text>
        <View style={{ width: 36 }} />
      </View>

      <View style={styles.subBar}>
        <Text style={styles.subBarText}>{packages.length} curated journeys</Text>
        <TouchableOpacity testID="add-pkg" style={styles.addBtn} onPress={() => router.push("/admin/package-form/new")}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.empty}>No packages yet. Tap "Add new" to curate one.</Text>}
        />
      )}
    </View>
  );
