...
interface Room {
    ...
    extra_guest_price: number;
    tax: number;
    // CHANGED: rooms no longer carry their own uploaded files. They
    // reference photos already uploaded for the property (previewImages)
    // by id, plus which one of those is the room's cover shot.
...
/* ---------------------------------------------------------
   DEMO SEED DATA (used only until real data is hydrated)
   NOTE: demo rooms start with no photos selected — there's nothing to
   sensibly pre-select since photo selection now depends on whatever the
   vendor actually uploads in Step 3. They pick photos live in Step 7.
--------------------------------------------------------- */
        ...
        <div>
            <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[11px] text-[#9A917D] font-mono-num">
                    {selectedIds.length}/{MAX_ROOM_PHOTOS} selected
                </span>
            </div>
    ...
    const savePhotos = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                // temporary local `uid()` ids past this point.
