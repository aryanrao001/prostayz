...
// Curated icon set — matches names stored in amenities.icon exactly,
// since they're real lucide-react export names.
...
function AmenityIcon({ icon, className }: { icon: string | null; className?: string }) {
...
type DrawerProps = {
  ...
  onClose: () => void;
  onSaved: (amenity: Amenity, mode: "create" | "edit") => void;
};

function AmenityDrawer({ mode, initial, onClose, onSaved }: DrawerProps) {
    ...
    try {
      if (mode === "create") {
        const { data } = await axios.post(
          `${backendUrl}/api/admin/amenities`,
          { name: name.trim(), icon, status },
          { withCredentials: true }
        );
        if (!data.success) throw new Error(data.message || "Could not create amenity");
        onSaved(data.amenity, "create");
      } else if (initial) {
        const { data } = await axios.put(
          `${backendUrl}/api/admin/amenities/${initial.id}`,
          { name: name.trim(), icon, status },
          { withCredentials: true }
        );
        if (!data.success) throw new Error(data.message || "Could not update amenity");
        onSaved(data.amenity, "edit");
      }
      onClose();
    } catch (err: any) {
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#E5DECF] bg-white/60">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#C99A3D] mb-0.5">
              {mode === "create" ? "New amenity" : "Edit amenity"}
            </p>
            <h3 className="font-display text-[19px] text-[#1E2A23]">
              {mode === "create" ? "Add amenity" : initial?.name}
            </h3>
          </div>
          <button
            onClick={onClose}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-2xl border border-[#E5DECF] bg-[#F5F2EA] shadow-2xl p-6">
        <h3 className="font-display text-[18px] text-[#1E2A23] mb-2">Delete "{amenity.name}"?</h3>
        <p className="text-[13px] text-[#6B6354] mb-4 leading-relaxed">
          This can't be undone. If properties currently list this amenity, deletion will be blocked — mark it inactive instead.
        </p>
        {error && (
  ...
  return (
    <div className="rounded-xl border border-[#E5DECF] bg-white/70 p-4 flex items-center gap-3">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[13.5px] font-semibold text-[#1E2A23] truncate">{amenity.name}</p>
...
const Amenities = () => {
  ...
  const [drawer, setDrawer] = useState<{ mode: "create" | "edit"; amenity: Amenity | null } | null>(null);
  ...
  const fetchAmenities = useCallback(async () => {
    ...
    setError(null);
    try {
      const { data } = await axios.get<ListResponse>(`${backendUrl}/api/admin/amenities`, {
        params: { search, status },
        withCredentials: true,
      });
      if (!data.success) throw new Error(data.message || "Could not load amenities");
      setAmenities(data.amenities);
    } catch (err: any) {
      setError(err?.response?.data?.message || err.message || "Could not load amenities");
      setAmenities([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleToggle = async (amenity: Amenity) => {
    ...
    const nextStatus = amenity.status ? 0 : 1;
    setAmenities((prev) => prev.map((a) => (a.id === amenity.id ? { ...a, status: nextStatus } : a)));
    try {
      const { data } = await axios.put(
        `${backendUrl}/api/admin/amenities/${amenity.id}`,
        { status: !!nextStatus },
        { withCredentials: true }
      );
      if (!data.success) throw new Error(data.message || "Could not update amenity");
      ...
      // revert on failure
      setAmenities((prev) => prev.map((a) => (a.id === amenity.id ? { ...a, status: amenity.status } : a)));
    } finally {
      setTogglingId(null);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setDeleteError(null);
    try {
      const { data } = await axios.delete(`${backendUrl}/api/admin/amenities/${deleteTarget.id}`, {
        withCredentials: true,
      });
      if (!data.success) throw new Error(data.message || "Could not delete amenity");
      setAmenities((prev) => prev.filter((a) => a.id !== deleteTarget.id));
      setDeleteTarget(null);
    } catch (err: any) {
      setDeleteError(err?.response?.data?.message || err.message || "Could not delete amenity");
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div style={{ fontFamily: "'Inter', ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6">
        <div>
          ...
          <h2 className="font-display text-[24px] text-[#1E2A23]">Amenities</h2>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            onClick={() => setDrawer({ mode: "create", amenity: null })}
      ...
      {!loading && !error && amenities.length === 0 && (
        <div className="rounded-2xl border border-dashed border-[#DBD3C4] bg-white/40 flex flex-col items-center justify-center py-20 text-center">
          <Sparkles size={26} className="text-[#B3AB99] mb-3" />
          <p className="text-[13.5px] font-medium text-[#1E2A23] mb-1">No amenities found</p>
          <p className="text-[12px] text-[#9A917D] mb-4">
            {search || status ? "Try a different search or filter." : "Add your first amenity to get started."}
          </p>
          {!search && !status && (
            <button
              onClick={() => setDrawer({ mode: "create", amenity: null })}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {amenities.map((a) => (
            <AmenityCard
              ...
              onEdit={() => setDrawer({ mode: "edit", amenity: a })}
              onDelete={() => {
                setDeleteError(null);
                setDeleteTarget(a);
              }}
      ...
      {drawer && (
        <AmenityDrawer
          mode={drawer.mode}
          initial={drawer.amenity}
          onClose={() => setDrawer(null)}
          onSaved={(amenity, mode) => {
            setAmenities((prev) =>
              mode === "create" ? [amenity, ...prev] : prev.map((a) => (a.id === amenity.id ? amenity : a))
            );
          }}
        />
      )}
